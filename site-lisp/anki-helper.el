...
;;;;;;;;;;;;;;;;;; my customization of cloze card ;;;;;;;;;;;;;;;;;;;;;;;;;

(defun +anki-helper--make-cloze ()
  (let* ((word
          (if (region-active-p)
         ...
         (escaped-word (regexp-quote word))
         (modified-sentence (replace-regexp-in-string
                             (concat "\\b" escaped-word "\\b")
                             (format "{{c1::%s}}" word)
                             sentence
                             t t)))
    ;;    (message "THE WORD is: %s \n THE SENTENCE is: %s \n" word sentence)
    ;;    (message "%s" modified-sentence)
    (list modified-sentence '())))

(defcustom +anki-helper-fields-get-alist
  '(("Basic" . anki-helper-fields-get-default)
    ("Cloze" . +anki-helper--make-cloze))
  "Alist of (NOTE-TYPE . FUNCTION) pairs.
...
(defun +anki-helper--get-fields (note-type)
  "Get the fileds info of the current entry for NOTE-TYPE."
  (let ((fields (anki-helper--get-note-fields note-type))
        ;; (field-contents (+anki-helper--make-cloze))
        (field-contents (funcall (alist-get
                                  note-type
                                  +anki-helper-fields-get-alist
                                  #'anki-helper-fields-get-default
...
(defun +anki-helper--get-cloze-content ()
  "Create an `anki-helper--note' struct for current Anki entry."
  (let* ((note-type (if (string= anki-helper-default-deck "辞海") "Basic" "Cloze"))
         (fields (+anki-helper--get-fields note-type))
         ;;         (maybe-id (org-entry-get nil anki-helper-prop-note-id))
         (deck (anki-helper--find-prop
                anki-helper-prop-deck
                anki-helper-default-deck))
...
;;;###autoload
(defun +anki-helper-capture-cloze-card ()
  (interactive)
  (when-let* ((body (anki-helper--transform-notes (list(+anki-helper--get-cloze-content)))))
    (+anki-helper-request 'addNotes
                          body
                          )))
...
(defun +anki-helper-request (action body &optional info)
  (let* ((args (anki-helper--request-args action body))
         (process (apply #'start-process
                         ...
                         "curl"
                         args)))
    (message "post to anki success")
    ))

(defun anki-select-deck ()
  (interactive)
  (let ((selected-deck (read-string "select a deck: ")))
    (setq anki-helper-default-deck selected-deck)
    (message "Set deck to `%s'" selected-deck)))

;; (defun region-contains-han-script-p (start end)
;;   "Return t if any character in region START to END belongs to Han script."
;;   (catch 'found
;;     (save-excursion
;;       (goto-char start)
;;       (while (< (point) end)
;;         (let ((char (char-after)))
;;           (when (and char
;;                      (eq (get-char-code-property char 'script) 'han))
;;             (throw 'found t)))
;;         (forward-char 1))
;;       nil)))

;; (defun check-region-for-han ()
;;   "Check if the active region contains Han script characters and message the result."
;;   (interactive)
;;   (if (use-region-p)
;;       (if (region-contains-han-script-p (region-beginning) (region-end))
;;           (message "Region contains Han script characters.")
;;         (message "Region does NOT contain Han script characters."))
